...
# Phase 2 — Just Train Twice (JTT) Comparison
To place this work in the current debiasing literature, we implemented Just Train Twice (Liu et al., ICML 2021), the standard no-group-labels baseline. JTT trains a short ERM identifier model to find which training samples resist the shortcut, then upsamples those failures and retrains from scratch. Hyperparameters used: T_id=1 epoch, λ_up=50.
Results at bias=0.8:
MethodShortcut ScoreTest AccSC ReductionBaseline0.536897.53%—ColorJitter0.638798.86%−19% (worse)HighDropout0.516197.10%3.9%JTT0.533397.50%0.6%Suppression (ours)0.102596.71%80.9%
JTT found 2,432 failure samples (4.1% of training data) after one identifier epoch and upsampled them to constitute 68% of each training batch. Despite this aggressive reweighting, its shortcut score barely moved (0.6% reduction) while achieving accuracy on par with baseline. The reason is structural: JTT reweights which samples the model sees but places no constraint on where in the image the model looks. A model retrained on upsampled wrong-color samples can still solve them by attending to color — the wrong color still provides information about what the correct color is not. Shortcut reliance in the spatial sense is untouched.
Our method achieves 80.9% shortcut score reduction at a cost of 0.82 percentage points of accuracy. The key difference is that our penalty operates directly in attribution space during gradient computation, targeting the spurious feature geometrically rather than statistically. JTT and our method are therefore solving different problems: JTT improves worst-group accuracy by rebalancing the training distribution, while our method explicitly suppresses the spatial attention pattern that constitutes the shortcut, producing an attribution audit trail as a byproduct of training.
The ColorJitter result is the sharpest negative finding: shortcut score increases from 0.5368 to 0.6387 while accuracy improves. Color augmentation teaches the model to handle color variation more carefully, which means attending to color more precisely — the opposite of the intended effect. This confirms that implicit distribution augmentation is not equivalent to explicit attribution-based suppression.
